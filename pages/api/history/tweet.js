...
export default async (req, res) => {
  ...
  //   bearerToken: token.account.access_token
  // })

  const second_client = new TwitterApi({ bearerToken: token.account.access_token })

  const twitter = second_client.readWrite;

  try {

    const user = await Users.findOne({ twitterId: token.sub });
    ...
    console.error("errordata",error.data.errors);
    console.error('Body:', body);
    return res.status(400).send({ success: false });
  }
};
